...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen pt-10 w-full flex flex-col items-center justify-start bg-gradient-to-br from-pink-200 via-blue-100 to-yellow-100 overflow-hidden">
      ...
      {/* Modern header: left text, right image, purple-pink bg */}
      <header className="max-w-6xl mx-auto mb-30 flex flex-row items-center justify-between w-full h-72 mt-34 md:h-96 rounded-3xl relative overflow-hidden">
        <div className="flex-1 flex flex-col items-start justify-center pl-8 z-10">
          <span className="text-5xl md:text-6xl font-extrabold tracking-tight bg-gradient-to-r from-purple-600 via-pink-500 to-yellow-400 bg-clip-text text-transparent drop-shadow-2xl animate-pulse">imprezza</span>
          <p className="mt-4 text-xl md:text-2xl font-semibold text-white/90 drop-shadow-lg tracking-wide">The new address of art</p>
          <p className="mt-4 text-xl md:text-xl font-semibold text-white/90 drop-shadow-lg tracking-wide">Imprezza is a vibrant platform connecting talented artists with art lovers and buyers worldwide. Discover unique artworks, connect with creators, and be part of a thriving global art community. Whether you're looking to buy, sell, or simply appreciate art, Imprezza welcomes you.</p>
        </div>
        <div className="flex-1 flex items-center justify-end pr-8 z-10">
          <img
            src="/image.png"
            alt="Artistic visual"
            className="w-60 h-60 md:w-150 md:h-150 object-cover"
          />
        </div>
      ...
      {/* Question and cards */}
      <section className="w-full flex flex-col mb-10 items-center justify-center pt-2 pb-6">
        ...
        <div className="w-full max-w-4xl flex flex-row items-stretch gap-10 px-4">
          {/* Buyer Card */}
          <div className="flex-1 flex flex-col items-center justify-center rounded-2xl shadow-lg mx-2 hover:scale-105 hover:shadow-2xl transition-all duration-200 cursor-pointer group" onClick={() => handleSelect("artist") }>
          {/* <div className="flex-1 flex flex-col items-center justify-center bg-gradient-to-tl from-blue-100 via-white to-blue-200 rounded-2xl shadow-lg p-8 mx-2 border-2 border-blue-300/30 hover:scale-105 hover:shadow-2xl transition-all duration-200 cursor-pointer group" onClick={() => handleSelect("buyer") }> */}
            ...
            <button className="w-full py-3 px-6 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-lg font-semibold shadow group-hover:shadow-xl">Find Talent</button>
          </div>
          {/* Artist Card */}
          <div className="flex-1 flex flex-col items-center justify-center rounded-2xl shadow-lg mx-2 hover:scale-105 hover:shadow-2xl transition-all duration-200 cursor-pointer group" onClick={() => handleSelect("artist") }>
          {/* <div className="flex-1 flex flex-col items-center justify-center bg-gradient-to-tl from-green-100 via-white to-green-200 rounded-2xl shadow-lg p-8 mx-2 border-2 border-green-300/30 hover:scale-105 hover:shadow-2xl transition-all duration-200 cursor-pointer group" onClick={() => handleSelect("artist") }> */}
            ...
            <button className="w-full py-3 px-6 bg-green-600 text-white rounded-lg hover:bg-green-700 transition text-lg font-semibold shadow group-hover:shadow-xl">Join as Arist</button>
          </div>
        </div>
      </section>
